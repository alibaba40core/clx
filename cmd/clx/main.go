...
func printHelp(w io.Writer) {
	...
	fmt.Fprintln(w)
	fmt.Fprintln(w, "Aliases (~/.clx/aliases.yaml):")
	fmt.Fprintln(w, "  User shortcuts expand at parse time (first token only), then follow the")
	fmt.Fprintln(w, "  same rules, risk, policy, and safety gates as any other input.")
	fmt.Fprintln(w, "  clx alias set gst \"git status\"   define shortcut gst → git status")
	fmt.Fprintln(w, "  clx alias list                   show saved aliases")
	fmt.Fprintln(w, "  clx alias rm gst                 remove alias")
	fmt.Fprintln(w, "  clx gst                          run expanded command (or clx --explain gst)")
	fmt.Fprintln(w)
