...
func continueAfterFastMiss(ctx context.Context, cfg config.Config, opts *Options, eng *intent.Engine, req parser.Request, profile environment.SystemProfile, lazyAliases *aliases.LazyLookup, aliasLookup parser.AliasLookup) (int, error) {
	return runFullResolveLite(ctx, cfg, opts, eng, req, profile, lazyAliases, aliasLookup)
}

func runFullResolveLite(ctx context.Context, cfg config.Config, opts *Options, eng *intent.Engine, req parser.Request, profile environment.SystemProfile, lazyAliases *aliases.LazyLookup, aliasLookup parser.AliasLookup) (int, error) {
	...
	if err != nil {
		if isResolverMiss(err) {
			...
			if needsAIWorker(cfg) {
				return delegateToWorker(ctx, opts)
			}
		}
...
func needsAIWorker(cfg config.Config) bool {
	if aiEnabled(cfg) {
		return true
	}
	return cfg.Features.AICommandGeneration
}
