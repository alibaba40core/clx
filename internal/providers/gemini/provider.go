...
// Provider implements providers.Provider via the Google Gemini generateContent API.
type Provider struct {
	client *Client
}

// NewProvider wires apiKey, model, and HTTP timeout. Does not contact Gemini until ResolveIntent.
...
// ResolveIntent builds the global prompt, schema, and calls Gemini generateContent.
func (p *Provider) ResolveIntent(ctx context.Context, req providers.IntentRequest) (*providers.IntentResponse, error) {
	...
	// Gemini's responseSchema uses OpenAPI 3.0 subset; strip unsupported fields.
